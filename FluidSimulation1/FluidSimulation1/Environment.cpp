...
bool checkCollision(Particle particle, Surface2D obstacle) {
	return false;
}

// Function to compute the squared distance between two points
float squared_distance(const Point& p1, const Point& p2) {
	float dx = p1.X - p2.X;
	float dy = p1.Y - p2.Y;
	return dx * dx + dy * dy;
}
...
bool check_line_segment_circle_intersection(const Point& A, const Point& B, const Point& C, float radius) {
