...
void Environment::renderParticles(int width, int height) {
	for (auto& particle : m_Particles) {
		...
		Graphics::velocityToColor(particle->m_Velocity.magnitude(), red, green, blue);

		glColor4f(red, green, blue, 1.0f);
		Graphics::DrawCircle(width, height, particle->m_Position.X, particle->m_Position.Y, particleRadius * 2, 20);
